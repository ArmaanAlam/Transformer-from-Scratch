...
- **Memory-efficient data pipeline** — trains BPE tokenizers and pre-tokenizes an arbitrarily large CSV (e.g. an 8GB corpus) by streaming in chunks and writing to disk-backed `.npy` memmaps, so RAM usage stays flat regardless of dataset size.
- **Mixed-precision training** with gradient accumulation, gradient clipping, cosine LR scheduling, checkpoint/resume support.
...
├── inference.py         # Thin wrapper meant to expose load_model()/translate() to the API
├── fastapi.py            # FastAPI server exposing POST /translate
...
checkpoints/best_model.pt, last.pt, config.json
        │  fastapi.py (loads model + tokenizers once at startup)
        ▼
...
## ⚠️ Known issue to fix before serving

`fastapi.py` imports `load_model` and `translate` from **`inference.py`**:

```python
from inference import load_model, translate
```

But `inference.py` currently only contains dummy placeholders:
...
The **real** implementations of `load_model()` and `translate()` (which build the `Transformer`, load tokenizers, load checkpoint weights, encode/decode sentences, and run `greedy_decode`) live in `test.py`, not `inference.py`.

**Fix:** either
1. Copy the `load_model`, `encode_sentence`, `decode_tokens`, and `translate` functions from `test.py` into `inference.py`, or
2. Change the import in `fastapi.py` to `from test import load_model, translate`.
...
`requirements.txt` currently covers the modeling/data stack:
...
`fastapi`, `uvicorn`, `streamlit`, and `requests` are used by `fastapi.py`/`app.py` but aren't listed yet — add them to `requirements.txt` or install them separately as shown above.
...
- `--resume` flag resumes from `last.pt` if present.
- `--max_samples N` lets you train on a subset for quick debugging.
...
First apply the fix described above (point `fastapi.py` at real `load_model`/`translate`), then:

```bash
uvicorn fastapi:app --host 127.0.0.1 --port 8000
```

This loads the model once at startup (`checkpoints/` + `tokenizers/` by default) and exposes:
...
> Note: `fastapi.py` is a Python module named `fastapi.py`, which **shadows the real `fastapi` package** it also tries to import (`from fastapi import FastAPI`). Rename it (e.g. `server.py` or `main.py`) to avoid an import collision, and update the `uvicorn` command accordingly (e.g. `uvicorn server:app`).
...
These sinusoidal functions allow the model to capture the order of words while generalizing to sequence lengths not seen during training.
...
| fastapi     | Inference API (not yet in requirements.txt) |
